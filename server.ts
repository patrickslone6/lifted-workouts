...
const readAccounts = () => { try { return JSON.parse(fs.readFileSync(ACCOUNTS_FILE, 'utf8') || '{}'); } catch { return {}; } };
const writeAccounts = (data: any) => { try { fs.writeFileSync(ACCOUNTS_FILE, JSON.stringify(data, null, 2), 'utf8'); } catch (e) { console.error(e); } };
const today = () => new Date().toISOString().slice(0, 10);

let ai: GoogleGenAI | null = null;
if (process.env.GEMINI_API_KEY) {
  try { ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY }); } catch (e) { console.warn('Gemini initialization failed:', e); }
}
...
function safeFallbackWorkout(body: any) {
  ...
  const exercises = hasPractice ? [
    ['DB Bench Press', 3, 8, 0], ['Chest-Supported DB Row', 3, 10, 0], ['Seated DB Press', 2, 10, 0], ['Cable Pallof Press', 3, 10, 0], ['Hip Mobility Flow', 2, 30, 0]
  ] : [
    ['Goblet Squat', 3, 8, 0], ['DB Romanian Deadlift', 3, 10, 0], ['DB Bench Press', 3, 8, 0], ['Chest-Supported DB Row', 3, 10, 0], ['Split Squat', 2, 8, 0], ['Pallof Press', 2, 10, 0]
  ];
  return { id: `fallback-${Date.now()}`, date, workoutTitle: hasPractice ? 'Practice-Smart Strength Session' : 'Adaptive Athletic Strength Session', goal: user.goals?.join(', ') || 'Strength and athletic performance', status: 'planned', estimatedMinutes: readiness.availableMinutes || 35, readinessStatus: hasPractice ? 'Moderate Readiness' : 'High Readiness', reasoning: hasPractice ? 'Practice is part of today\'s load, so the plan preserves freshness and avoids unnecessary lower-body fatigue.' : 'No practice was reported today, so the plan can use the available training time for a normal progressive strength stimulus.', practiceLaterToday: hasPractice, injuryProtectionNotes: injured.length ? `The plan should be adjusted around the reported concern: ${injured.join(', ')}. Stop any movement that causes pain and use the listed alternative.` : 'No specific injury restrictions were reported.', isAiGenerated: false, equipmentNeeded: ['dumbbell','bench','cable'], exercises: exercises.map(([name, sets, reps], i) => ({ id: `fb-${i}`, exerciseId: `fb-${i}`, name, sets, reps, recommendedWeight: 0, weightUnit: user.weightUnit || 'lb', restSeconds: 75, targetMuscles: ['Full Body'], notes: 'Choose a comfortable load that allows controlled technique and leaves a few good reps in reserve.', whyWeightHypertrophyInjury: 'Load should be adjusted to technique, recent performance, readiness, and any pain; do not force a number.', completed: false, alternative: 'Choose a pain-free equivalent movement.' })) };
}

app.get('/api/health', (_req, res) => res.json({ status: 'ok', hasGeminiKey: Boolean(process.env.GEMINI_API_KEY), ai: Boolean(ai) }));
...
app.post('/api/cloud-sync/save', (req, res) => {
  try {
    ...
    existing.userData = req.body.data || null; existing.lastSyncedAt = new Date().toISOString(); accounts[id] = existing; writeAccounts(accounts);
...
app.post('/api/generate-plyometrics', async (req, res) => {
  ...
  } catch { return res.status(503).json({ error: 'AI plyometrics unavailable' }); }
});
...
app.post('/api/coach-chat', async (req, res) => {
  ...
  } catch { return res.json({ success: true, reply: 'I can still coach from the data saved in Lifted. Focus on controlled technique, recover between hard activities, and do not push through pain. Reopen this question when the AI service is available.', actionItems: [], relevantData: [] }); }
});
...
app.post('/api/exercise-summary', async (req, res) => {
  ...
  } catch { return res.status(503).json({ error: 'AI exercise summary unavailable' }); }
});
...
app.post('/api/weekly-summary', async (req, res) => {
  try {
    ...
    return res.json({ success: true, ...(await askAI(prompt)) });
  } catch { return res.status(503).json({ error: 'AI weekly summary unavailable' }); }
});
