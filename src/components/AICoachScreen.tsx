  ...
  const send = async (preset?: string) => {
    ...
    try {
      ...
      const response = await apiFetch('/api/coach-chat', {
        ...
        body: JSON.stringify({
          message: text,
          history: next.slice(-60),
          user,
          readiness: completeReadiness,
          recentHistory: completeHistory.slice(0, 30),
          scheduledEvents,
          recentSchoolLogs: completeSchoolLogs.slice(0, 30),
          todayWorkout: currentPlan,
          customFocus: `Recent Coach conversation: ${JSON.stringify(next.slice(-40))}`,
      ...
      persist([...next, {
        id: `bot-${Date.now()}`,
        sender: 'assistant',
        text: 'I could not reach Coach AI right now. Your message was saved locally. Please try again when the connection is available.',
        timestamp: new Date().toISOString(),
      }]);
    } finally {
      setLoading(false);
    ...
    <section className="coach-shell" aria-label="Lifted AI Coach">
      ...
      <div className="coach-messages">
        ...
        {messages.map((m) => {
          ...
          return <div key={m.id} className={`coach-row ${mine ? 'mine' : ''}`}><div className={`coach-avatar ${mine ? 'user-avatar' : ''}`}>{mine ? <User size={15} /> : <Bot size={15} />}</div><div className={`coach-bubble ${mine ? 'user-bubble' : ''}`}><div>{safeText(m.text)}</div><time>{new Date(m.timestamp).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</time></div></div>;
        })}
      ...
      <form className="coach-composer" onSubmit={(e) => { e.preventDefault(); void send(); }}>
        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Lifted anything about your training…" autoComplete="off" enterKeyHint="send" disabled={loading} />
