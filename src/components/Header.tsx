      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-400 via-emerald-500 to-teal-600 flex items-center justify-center text-black font-black shadow-lg shadow-emerald-950">
          ...
          {/* Practice / Game notification pills & Actions */}
          <div className="flex items-center gap-2">
            {onOpenCustomWorkout && (
              <button
                onClick={onOpenCustomWorkout}
                className="px-3 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-bold transition flex items-center gap-1.5 shadow-md shadow-emerald-950 shrink-0"
              >
            ...
            {hasPracticeToday && (
              <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/10 text-amber-300 border border-amber-500/25">
                <AlertCircle className="w-3 h-3" /> Practice Later
              </div>
            )}
            ...
            <button
              ...
              title={currentAccount ? `Synced: ${currentAccount.identifier}` : 'Sync Account'}
            >
              <UserCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span className="hidden sm:inline">{currentAccount ? 'Synced' : 'Sync'}</span>
            </button>

            <button
              onClick={onOpenCheckIn}
              className="p-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 transition"
        ...
        {/* Tab Navigation */}
        <nav className="flex space-x-1 sm:space-x-2 overflow-x-auto py-2 border-t border-zinc-900 scrollbar-none">
          {[
